    ...
    public void run() {
        ...
        while (session != null && session.connected) {
            try {
                ...
                // Silent catch - queue rỗng
            }
            try {
                Thread.sleep(10);
            } catch (Exception e) {
                // Thread bị interrupt khi disconnect
                break;
            }
        }
    }
...
//            Client.gI().kickSession(session);
    ...
    public void sendSessionKey() {
        ...
        try {
            ...
            // FIX: Thay vì session.sendThread.start()
            // Gọi method mới để submit sender vào thread pool
            session.startSenderThread();
        } catch (Exception e) {
            Log.error(MessageSender.class, e);
        }
    }
