...
public class PhucLoi {
    ...
    public int countPlayer(Player pl, PhucLoi phucloi) {
        switch (phucloi.tab_id) {
            ...
            case 2: {
                phucloi.count = pl.getSession().tongnap1;
                break;
            }
            case 3: {
                phucloi.count = pl.tongnap;
                break;
            }
             case 4: {
            // Lấy điểm quay của vòng quay đế vương
            phucloi.count = PlayerDAO.getDiemQuayByPlayerId((int) pl.id);; // Nếu đã có sẵn trong Player object
            // HOẶC nếu cần query từ DB:
            //phucloi.count = PlayerDAO.getDiemQuayByPlayerId((int) pl.id);
            break;
        }
            default:
                phucloi.count = 0;
    ...
    public void Active_PhucLoi(Player pl, int tabid) {
        ...
        if (phucloi.active == 0 || phucloi.active == 2) {
                    ...
                    if (countPlayer(pl, phucloi) >= phucloi.max_count) {
                        if (phucloi.PHUCLOI_LIST_ITEM.size() < InventoryService.gI().getCountEmptyBag(pl)) {
                            ...
                            for (int j = 0; j < phucloi.PHUCLOI_LIST_ITEM.size(); j++) {
                                Item itemGoc = phucloi.PHUCLOI_LIST_ITEM.get(j);
            ...
            if (phucloi.tab_id == 1) {
                if (countPlayer(pl, phucloi) >= phucloi.max_count) {
                    if (phucloi.PHUCLOI_LIST_ITEM.size() < InventoryService.gI().getCountEmptyBag(pl)) {
//                        if (!Player.vn.(pl, phucloi.max_count)) {
