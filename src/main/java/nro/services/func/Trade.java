...
public class Trade {
    ...
    public void openTabTrade() {
        // KIỂM TRA CẢ 2 NGƯỜI ĐỀU PHẢI MỞ THÀNH VIÊN
        if (!player1.getSession().actived || !player2.getSession().actived) {
            String notActive = !player1.getSession().actived ? player1.name : player2.name;
            Service.getInstance().sendThongBaoFromAdmin(player1, 
                "|5|Không thể giao dịch! " + notActive + " chưa kích hoạt tài khoản tại " + Manager.DOMAIN);
            Service.getInstance().sendThongBaoFromAdmin(player2, 
                "|5|Không thể giao dịch! " + notActive + " chưa kích hoạt tài khoản tại " + Manager.DOMAIN);
    ...
    public void addItemTrade(Player pl, byte index, int quantity) {
        ...
        Player otherPlayer = pl.equals(player1) ? player2 : player1;
        
        // KIỂM TRA NGƯỜI THÊM ĐỒ PHẢI MỞ THÀNH VIÊN
        ...
        // KIỂM TRA NGƯỜI NHẬN ĐỒ CŨNG PHẢI MỞ THÀNH VIÊN
        if (!otherPlayer.getSession().actived) {
            Service.getInstance().sendThongBaoFromAdmin(pl,
                "|5|Không thể giao dịch! " + otherPlayer.name + " chưa kích hoạt tài khoản");
            Service.getInstance().sendThongBaoFromAdmin(otherPlayer,
                "|5|Không thể nhận vật phẩm! Vui lòng kích hoạt tài khoản tại " + Manager.DOMAIN);
            removeItemTrade(pl, index);
            cancelTrade();
            return;
        }
    ...
    public void lockTran(Player pl) {
        ...
        if (!player1.getSession().actived || !player2.getSession().actived) {
            String notActive = !player1.getSession().actived ? player1.name : player2.name;
            Service.getInstance().sendThongBaoFromAdmin(player1, 
                "|5|Giao dịch bị hủy! " + notActive + " chưa kích hoạt tài khoản");
            Service.getInstance().sendThongBaoFromAdmin(player2, 
                "|5|Giao dịch bị hủy! " + notActive + " chưa kích hoạt tài khoản");
            cancelTrade();
            return;
    ...
    private void startTrade() {
        // KIỂM TRA LẦN CUỐI TRƯỚC KHI HOÀN TẤT GIAO DỊCH
        if (!player1.getSession().actived || !player2.getSession().actived) {
            String notActive = !player1.getSession().actived ? player1.name : player2.name;
            Service.getInstance().sendThongBao(player1, 
                "Giao dịch thất bại: " + notActive + " chưa kích hoạt tài khoản");
