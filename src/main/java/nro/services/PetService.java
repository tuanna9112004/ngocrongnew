...
public class PetService {
    ...
    // Thread pool duy nhất cho tất cả pet tasks (CHỈ 1 thread)
    private final ExecutorService petExecutor;

    private PetService() {
        // Tạo thread pool với 1 thread duy nhất vì pet tasks ngắn
    ...
    public void createLuffyPet(Player player, byte... limitPower) {
        petExecutor.submit(() -> {
            try {
                ...
                Thread.sleep(1000);
                Service.getInstance().chatJustForMe(player, player.pet, "Ta là Thẩy của vỹ thú itachi...");
            } catch (Exception e) {
            }
        });
    }
    ...
    public void createLuffyPet(Player player, int gender, byte... limitPower) {
        petExecutor.submit(() -> {
            try {
                ...
                Thread.sleep(1000);
                Service.getInstance().chatJustForMe(player, player.pet, "Ta là Thầy vỹ thú itaci...");
            } catch (Exception e) {
            }
        });
    }
    ...
    public void createNewPet(Player player, boolean isMabu, boolean isBerus, boolean isPic, boolean isLuffy, byte... gender) {
        ...
        pet.name = "$" + (isMabu ? "Mabư" : isBerus ? "Berus" : isPic ? "Thần Zeno" : isLuffy ? "itachi " : "Đệ tử");
