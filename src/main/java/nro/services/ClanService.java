...
public void load() {
    try {
        ...
        PreparedStatement ps = con.prepareStatement("SELECT * FROM clan_sv" + Manager.SERVER);
        ...
        while (rs.next()) {
            ...
            clan.createTime = (int) rs.getLong("create_time");
            clan.bonusTime = (int) rs.getLong("BonusTime");
        ...
        clan.members.clear();
            ...
            if (membersJson != null && !membersJson.isEmpty()) {
                JSONArray jarr = (JSONArray) JSONValue.parse(membersJson);
                for (Object obj : jarr) {
                    JSONObject jobj = (JSONObject) obj;
                    ClanMember cm = new ClanMember();
                    cm.id = Integer.parseInt(jobj.get("id").toString());
                    cm.powerPoint = Long.parseLong(jobj.get("power").toString());
                    cm.name = (String) jobj.get("name");
                  cm.head = (short) Integer.parseInt(((JSONObject) obj).get("head").toString());
                    cm.body = (short) Integer.parseInt(((JSONObject) obj).get("body").toString());
                    cm.leg  = (short) Integer.parseInt(((JSONObject) obj).get("leg").toString());

                    cm.role = Byte.parseByte(jobj.get("role").toString());
                    cm.donate = Integer.parseInt(jobj.get("donate").toString());
                    cm.receiveDonate = Integer.parseInt(jobj.get("receive_donate").toString());
                    cm.memberPoint = Integer.parseInt(jobj.get("member_point").toString());
                    cm.clanPoint = Integer.parseInt(jobj.get("clan_point").toString());
                    long loginTimeLong = Long.parseLong(jobj.get("login_time").toString());
                        cm.clanTime = (loginTimeLong > Integer.MAX_VALUE) ? Integer.MAX_VALUE : (int) loginTimeLong;

                    cm.timeAskPea = Long.parseLong(jobj.get("ask_pea_time").toString());
                    clan.members.add(cm);
                }
            }

            Manager.CLANS.add(clan);
            Log.log("LOAD CLAN: " + clan.name + " (" + clan.id + ")");
        }
        rs.close();
        ps.close();
        con.close();
    } catch (Exception e) {
        Log.error(ClanService.class, e, "Có lỗi khi load clan");
    }
}
...
public void buffClan(Player player) {
    // Nếu đã có buff thì thoát
    if (player.isBuffClan) {
        Service.getInstance().sendThongBao(player, "Bạn đã có buff clan rồi!");
        return;
    }

    // Đánh dấu có buff
    player.isBuffClan = true;

    // Lưu chỉ số gốc
    long sdGoc = player.nPoint.dameg;
    long hpGoc = player.nPoint.hpMax;
    long kiGoc = player.nPoint.mpMax;

    // Buff 20%
    player.nPoint.dameg = (long) (player.nPoint.dameg * 1.2);
    player.nPoint.hpMax = (long) (player.nPoint.hpMax * 1.2);
    player.nPoint.mpMax = (long) (player.nPoint.mpMax * 1.2);

    // Hồi đầy HP/KI để người chơi thấy ngay
    player.nPoint.hp = player.nPoint.hpMax;
    player.nPoint.mp = player.nPoint.mpMax;

    // Gửi cập nhật chỉ số cho client
    Service.getInstance().point(player);
  //  Service.getInstance().sendInfoHpMp(player);

    // Thông báo buff
    Service.getInstance().sendThongBao(player, "Bạn đã được buff SD, HP, KI +20% trong 10 phút!");

    // Sau 10 phút tự động xóa buff
    Timer timer = new Timer();
    timer.schedule(new TimerTask() {
        @Override
        public void run() {
            // Trả lại chỉ số gốc
            player.nPoint.dameg = sdGoc;
            player.nPoint.hpMax = hpGoc;
            player.nPoint.mpMax = kiGoc;
            ...
            // Cập nhật lại cho client
            Service.getInstance().point(player);
          //  Service.getInstance().sendInfoHpMp(player);

            // Xóa trạng thái buff
            player.isBuffClan = false;
            Service.getInstance().sendThongBao(player, "Buff clan đã hết hiệu lực!");
        }
    }, 10 * 60 * 1000); // 10 phút
}

    public void selectClanBuff(Player player, int type) {
        Clan clan = player.clan;
        if (clan == null) {
        ...
        if (!clan.isLeader(player)) {
            Service.getInstance().sendThongBao(player, "Chỉ bang chủ mới có thể chọn buff!");
            return;
        }

        int costGold = 0;
        int costGem = 0;
        int minutes = 60; // thời gian buff tồn tại 60 phút

        switch (type) {
            case 1: costGold = 50_000_000; break; // Ba lô
            case 2: costGem = 150; break; // Mai rùa
            case 3: costGem = 150; break; // Gậy
            case 4: costGem = 250; break; // Gậy phép
            default:
                Service.getInstance().sendThongBao(player, "Loại buff không hợp lệ!");
                return;
        }

        // check tiền
        if (player.inventory.gold < costGold || player.inventory.getGem() < costGem) {
            Service.getInstance().sendThongBao(player, "Không đủ vàng hoặc ngọc để buff!");
            return;
        }

        // trừ tiền
        if (costGold > 0) player.inventory.gold -= costGold;
        if (costGem > 0) player.inventory.subGem(costGem);
        PlayerService.gI().sendInfoHpMpMoney(player);

        // set buff
        clan.bonusType = type;
        clan.bonusTime = System.currentTimeMillis() + (minutes * 60 * 1000);

        // thông báo toàn bang
        for (Player pl : clan.membersInGame) {
            if (pl != null) {
                Service.getInstance().sendThongBao(pl,
                        "Bang hội đã kích hoạt buff [" + getClanBuffName(type) + "] trong " + minutes + " phút!");
            }
        }
    }

    public String getClanBuffName(int type) {
        switch (type) {
            case 1: return "Ba lô (+10% sức đánh, +5% hút HP/KI)";
            case 2: return "Mai rùa (+5% sức đánh)";
            case 3: return "Gậy (+8% sức đánh)";
            case 4: return "Gậy phép (+15% sức đánh)";
            default: return "Không có";
        }
    }

// --- Buff sát thương: phiên bản chính có baseDame
public int getClanDamage(Player player, int baseDame) {
    if (player == null || player.clan == null) return baseDame;
    Clan clan = player.clan;

    // hết hạn buff → tắt buff
    if (clan.bonusTime < System.currentTimeMillis()) {
        clan.bonusType = 0;
        return baseDame;
    }

    int dame = baseDame;
    switch (clan.bonusType) {
        case 1: // gói: +10% dame, +5% hút HP/KI
            dame += dame * 10 / 100;
            break;
        case 2: // ví dụ: +5%
            dame += dame * 5 / 100;
...
// Dame khi có buff từ clan
public long getClanDamage(Player player, long baseDamage) {
    if (player == null || player.clan == null) {
        return baseDamage;
    }
    Clan clan = player.clan;

    // Nếu hết hạn buff thì reset
    if (clan.bonusTime < System.currentTimeMillis()) {
        clan.bonusType = 0;
        return baseDamage;
    }
    ...
    switch (clan.bonusType) {
        case 1:
            damage += damage * 10L / 100L;
            break;
        case 2:
            damage += damage * 5L / 100L;
            break;
        case 3:
            damage += damage * 8L / 100L;
            break;
        case 4:
            damage += damage * 15L / 100L;
            break;
        default:
            break;
    }
...
// Hút HP khi có buff clan
public long getClanHutHP(Player player) {
    if (player.clan == null) {
        return 0L;
    }
    if (player.clan.bonusType == 1 && player.clan.bonusTime > System.currentTimeMillis()) {
        return 5L; // hút 5% HP
    }
    return 0L;
}

// Hút KI khi có buff clan
public long getClanHutKI(Player player) {
    if (player.clan == null) {
        return 0L;
    }
    if (player.clan.bonusType == 1 && player.clan.bonusTime > System.currentTimeMillis()) {
   ...
   public void close() {
    ...
    try {
        ...
        ps = con.prepareStatement("update clan_sv" + Manager.SERVER
                ...
                + "level = ?, members = ?, bonus_type = ?, bonus_time = ? where id = ? limit 1");
        ...
        for (Clan clan : Manager.CLANS) {
            ...
            // buff mới
            ps.setInt(8, clan.bonusType);
            ps.setLong(9, clan.bonusTime);
