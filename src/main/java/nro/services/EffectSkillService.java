...
public void sendEffectUseSkill(Player player, byte skillId) {
    Skill skill = getSkillSafe(player, skillId);
    if (skill == null) {
        Log.error("Player " + player.name + " (id=" + player.id + ") dùng skillId=" + skillId + " nhưng chưa học skill này.");
        return; // tránh IndexOutOfBounds
    }
...
//        player.effectSkill.timeAnTroi = timeAnTroi;
    ...
    private Skill getSkillSafe(Player player, int skillId) {
    Skill skill = SkillUtil.getSkillbyId(player, skillId);
    if (skill == null) {
        Log.error("Player " + player.name + " (id=" + player.id + ") chưa có skillId=" + skillId);
    }
    return skill;
}
   ...
   public void sendEffectMonkey(Player player) {
    Skill skill = getSkillSafe(player, Skill.BIEN_KHI);
    if (skill == null) {
        return; // tránh crash server
    }
    ...
    try {
        msg = new Message(-45);
        msg.writer().writeByte(6);
      msg.writer().writeLong(player.id);
