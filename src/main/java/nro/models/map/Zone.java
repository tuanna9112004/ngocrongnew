...
// Zone.java
public void broadcastAppearance(Player who) {
    try {
        if (who == null || who.zone != this) return;
        if (this.map.isMapOffline) return;

        // Làm tươi state server-side (inventory/head/body/leg...)
        Service.getInstance().player(who);

        short footId = -1;
        try {
            if (who.inventory != null
                    && who.inventory.itemsBody != null
                    && who.inventory.itemsBody.size() > 11) {
                Item foot = who.inventory.itemsBody.get(11);
                if (foot != null && foot.isNotNullItem() && foot.template != null) {
                    footId = (short) foot.template.id;
                }
            }
        } catch (Exception ignore) {}

        // Chỉ gửi cho NGƯỜI KHÁC (không gửi cho chính mình)
        synchronized (this.players) {
            for (Player viewer : this.players) {
                if (viewer == null || viewer == who) continue;
                infoPlayer(viewer, who);
                Service.getInstance().sendFlagPlayerToMe(viewer, who);
                Service.getInstance().sendFootRv(who, viewer, footId);
                for (int i = 0; i < 5; i++) {
                    try { nro.sendEff.SendEffect.getInstance().sendDanhHieuAll(viewer, who, i); } catch (Exception ignore) {}
                }
            }
        }
    } catch (Exception e) {
...
public void load_Me_To_Another(Player player) {
    try {
        ...
        Zone zone = player.zone; // luôn đồng bộ zone thật
        if (zone == null) return;

        if (zone.map.isMapOffline) {
            if (player.isPet && zone.equals(((Pet) player).master.zone)) {
                infoPlayer(((Pet) player).master, player);
            }
            return;
        }

        // Đảm bảo player tồn tại trong danh sách
        if (!zone.humanoids.contains(player)) {
            zone.humanoids.add(player);
        }

        int count = 0;
        synchronized (zone.humanoids) {
           

            for (Player pl : zone.humanoids) {
                if (pl == null || pl.equals(player)) continue;

                // chỉ bỏ qua boss và minipet, không bỏ player có linh thú
                if (pl.isBoss || pl.isMiniPet) continue;
    ...
    try {
        if (this.map.isMapOffline) {
            ...
            for (Player pl : this.humanoids) {
                if (pl != null && (pl.id == -player.id || pl instanceof TestDame)) {
                    infoPlayer(player, pl);
                    break;
                }
            }
            ...
            int count = 0;
            for (Player pl : this.humanoids) {
                if (pl == null || player.equals(pl)) continue;
        ...
        // ✅ Chỉ gửi pet follow 1 lần sau khi load xong
        PlayerService.gI().sendPetFollow(player);

    } catch (Exception e) {
       // System.err.println("[Zone.loadAnotherToMe] ❌ EXCEPTION:");
        e.printStackTrace();
        Log.error(MapService.class, e);
    }

    //System.out.println("[Zone.loadAnotherToMe] ========== KẾT THÚC ==========\n");
}
...
private void infoPlayer(Player plReceive, Player plInfo) {
    ...
    try {
        // GỬI MESSAGE CHÍNH (-5)
        //System.out.println("[Zone.infoPlayer] ▶ Tạo message -5...");
        msg = new Message(-5);
        msg.writer().writeInt((int) plInfo.id);
        
        if (plInfo.clan != null) {
            msg.writer().writeInt(plInfo.clan.id);
          //  System.out.println("[Zone.infoPlayer]   → Clan ID: " + plInfo.clan.id);
        } else if (plInfo.isBoss && ((Boss) plInfo).isMabuBoss) {
            msg.writer().writeInt(-100);
           // System.out.println("[Zone.infoPlayer]   → Mabu Boss");
        } else {
            msg.writer().writeInt(-1);
           // System.out.println("[Zone.infoPlayer]   → Không có clan");
        }
        
        int level = CaptionManager.getInstance().getLevel(plInfo);
        level = plInfo.isInvisible ? 0 : level;
        ...
        //System.out.println("[Zone.infoPlayer]   → Mount ID: " + plInfo.getMount());
        ...
        msg.cleanup();
     //   System.out.println("[Zone.infoPlayer] ✓ Message -5 đã gửi");
    } catch (Exception e) {
      //  System.err.println("[Zone.infoPlayer] ❌ Lỗi gửi message -5");
        e.printStackTrace();
        Log.error(MapService.class, e);
    }
    
    // GỬI FLAG
   // System.out.println("[Zone.infoPlayer] ▶ Gửi Flag...");
    Service.getInstance().sendFlagPlayerToMe(plReceive, plInfo);

    // GỬI FOOT (PHỤ KIỆN CHÂN)
    if (!plInfo.isBoss && !plInfo.isPet && !plInfo.isMiniPet && !(plInfo instanceof TestDame)) {
        if (plInfo.inventory.itemsBody.size() > 11 && plInfo.inventory.itemsBody.get(11).isNotNullItem()) {
          //  System.out.println("[Zone.infoPlayer] ▶ Gửi Foot item: " + plInfo.inventory.itemsBody.get(11).template.name);
            Service.getInstance().sendFootRv(plInfo, plReceive, (short) plInfo.inventory.itemsBody.get(11).template.id);
        } else {
           // System.out.println("[Zone.infoPlayer] ℹ Không có Foot item");
        }
    }

    // GỬI TITLE (DANH HIỆU CỐ ĐỊNH)
   // System.out.println("[Zone.infoPlayer] ▶ Gửi Title cố định (888-892, 900-903)...");
    ...
    Service.getInstance().sendTitleRv(plInfo, plReceive, (short) 901);
    Service.getInstance().sendTitleRv(plInfo, plReceive, (short) 902);
    Service.getInstance().sendTitleRv(plInfo, plReceive, (short) 903);
    
    // ✅ GỬI DANH HIỆU THỜI HẠN
   // System.out.println("[Zone.infoPlayer] ▶ Gửi Danh hiệu thời hạn (SendEffect)...");
    for (int i = 0; i < 5; i++) {
        try {
            SendEffect.getInstance().sendDanhHieuAll(plReceive, plInfo, i);
           // System.out.println("[Zone.infoPlayer]   → Danh hiệu " + i + " đã gửi");
        } catch (Exception e) {
           // System.err.println("[Zone.infoPlayer]   ❌ Lỗi gửi danh hiệu " + i);
            e.printStackTrace();
        }
    }
    ...
    try {
        if (plInfo.isDie()) {
          //  System.out.println("[Zone.infoPlayer] ▶ Player đang chết, gửi message -8...");
            msg = new Message(-8);
            msg.writer().writeInt((int) plInfo.id);
            msg.writer().writeByte(0);
            msg.writer().writeShort(plInfo.location.x);
            ...
            plReceive.sendMessage(msg);
            msg.cleanup();
          //  System.out.println("[Zone.infoPlayer] ✓ Message -8 đã gửi");
        }
    } catch (Exception e) {
       // System.err.println("[Zone.infoPlayer] ❌ Lỗi gửi message -8");
        e.printStackTrace();
        Log.error(MapService.class, e);
    }
    
    //System.out.println("[Zone.infoPlayer] ========== KẾT THÚC ==========\n");
}
