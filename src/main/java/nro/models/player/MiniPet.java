...
public void setName(int itemID) {
    try {
        Item item = ItemService.gI().createNewItem((short) itemID);
        if (item != null && item.template != null) {
            this.name = "" + item.template.name; // Thêm |5| để hiển thị màu xanh
        } else {
            this.name = "Mini Pet"; // Tên mặc định với màu xanh
        }
    } catch (Exception e) {
        this.name = "Mini Pet";
    }
